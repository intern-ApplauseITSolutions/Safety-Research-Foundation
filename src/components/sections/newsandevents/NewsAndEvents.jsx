...
const NewsAndEvents = ({ initialSection = 'events', initialMediaTab = 'videos' }) => {
  ...
  // Load BRACE News PDFs and Images (fallback)
  ...
  // Load MSIA Award images (fallback)
  ...
  // Load documents when Documents tab is active (fallback)
  useEffect(() => {
    if (mainSection === 'media' && mediaTab === 'documents' && documents.length === 0) {
  ...
  }, [mainSection, mediaTab, documents.length]);
          ...
          <>
            ...
            {!loading.media && mediaTab === 'documents' && (
              <div>
                ...
                {/* BRACE News Section */}
                {documentTab === 'brace' && (documents.length > 0 || braceNewsDocuments.length > 0) && (
                  <div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
                      {(documents.length > 0 ? documents.filter(d => d.category === 'BRACE News') : braceNewsDocuments).map((doc) => (
                        <a
                          key={doc.id}
                        ...
                        >
                          ...
                          <div className="w-full aspect-[3/4] bg-gray-50 relative">
                            {doc.type === 'image' || doc.file_url?.match(/\.(jpg|jpeg|png)$/i) ? (
                              <img 
                              ...
                              <iframe
                                src={`${doc.file_url || doc.file}#toolbar=0&navpanes=0&scrollbar=0&page=1&view=FitH&zoom=page-fit`}
                          ...
                          <div className="p-3 bg-white">
                            ...
                            <div className="flex items-center gap-2 mt-2">
                              <span className="bg-primary/10 text-primary px-2 py-1 rounded text-xs font-medium">
                                {doc.type === 'image' || doc.file_url?.match(/\.(jpg|jpeg|png)$/i) ? 'Image' : 'PDF'}
                              </span>
                ...
                {/* Empty State */}
                {documents.length === 0 && braceNewsDocuments.length === 0 && (
