...
const MediaNew = () => {
  ...
  const fetchMedia = async () => {
    try {
      ...
      try {
        response = await api.get('/media/list.php', {
          params: { limit: 100 }
        })
      } catch (primaryError) {
        console.warn('Primary media API failed, falling back to direct API:', primaryError)
        response = await directApi.get('/media/list.php', {
          params: { limit: 100 }
        })
      }

      if (!response?.data?.success) {
        throw new Error(response?.data?.message || 'Unable to fetch media')
      }

      setMedia(normalizeMediaResponse(response))
    } catch (error) {
      console.error('Error fetching media:', error)
      setMedia([])
      setErrorMessage('Unable to load media from the database right now.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const uploadFile = async (file) => {
    ...
    try {
      const response = await api.post('/upload.php', uploadFormData, {
        headers: {
